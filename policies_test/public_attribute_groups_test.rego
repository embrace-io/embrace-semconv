...
local_attribute(key) := {
	"key": key,
	"provenance": {"path": "/repo/model/emb/registry.yaml"},
}
...
test_passes_when_local_attribute_is_exported if {
	count(deny) == 0 with input as {"registry": {
		"attributes": [local_attribute("emb.user_session_id")],
		"attribute_groups": [local_group(["emb.user_session_id"])],
	}}
}
...
test_imported_group_does_not_export_a_local_attribute if {
	count(deny) == 1 with input as {"registry": {
		"attributes": [local_attribute("emb.user_session_id")],
		"attribute_groups": [imported_group(["emb.user_session_id"])],
	}}
}
...
test_mixed_registry_flags_only_the_unexported_local_attribute if {
	findings := deny with input as {"registry": {
		"attributes": [
			local_attribute("emb.user_session_id"),
			local_attribute("emb.session_part_id"),
	...
	finding.context.attribute_key == "emb.session_part_id"
}
...
test_finding_has_the_expected_shape if {
	...
	finding.context.attribute_key == "emb.user_session_id"
}
...
test_tolerates_missing_collections if {
	...
	count(deny) == 1 with input as {"registry": {"attributes": [local_attribute("emb.user_session_id")]}}
}
...
test_attribute_without_provenance_is_treated_as_local if {
	count(deny) == 1 with input as {"registry": {
		"attributes": [{"key": "emb.user_session_id"}],
		"attribute_groups": [],
	}}
}
