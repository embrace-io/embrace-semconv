...
e.g. `…/embrace-common/0.3.0` releases as tag `v0.3.0`. The release workflow and `make package` both
read it from there.
...
- **The registry's identity never changes.** Everything in `schema_url` before the version segment
  (`embrace.io/schemas/embrace-common`) is the registry's name; only the version moves. Weaver
  treats a new name as a different registry, not a new version, so consumers would end up with two
  unrelated registries. (It changed once, before there were outside consumers: `v0.1.0` and
  `v0.2.0` were published as `embrace.io/schemas/embrace`.)
